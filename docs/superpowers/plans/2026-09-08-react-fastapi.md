...
**Status:** 用户后续已授权代码实施，分支 `refactor/react-fastapi`，继承文档提交 `02fa576`。本轮只进行代码和静态检查，所有构建、测试、训练、服务与故障注入验收均未执行。原基线 `a66ea1bc6f1cf7a8c5512dc0454634b33304a0ab`，文档日期 2026-09-08。执行调整见 [ADR-0006](../../adr/0006-static-only-implementation.md)。
...
- C-07：保留用户行情、历史 CSV、旧 experiments.db 和既有实验；docs1/ 保持忽略；本次在一个新工作树、一个新分支编写代码并静态检查，不运行构建、测试、训练或服务，不推送远端。
- C-08：前端不接触服务器任意路径，不计算另一套绩效；API、CLI 和 worker 使用同一领域规则；合成演示和重放必须显式标注。

本计划最初依照“先不要编写代码”编写，当前按后续开发指令实施。下方原验收步骤保留作为后续执行清单；未勾选的动态检查不代表没有编写相关代码，也不得把代码存在理解为验收已通过。代码提交留在本地，推送按用户后续指令处理。

## 1. 任务依赖与交付阶段

本次代码交付状态与下方动态验收复选框分别记录：

| 任务 | 已编写的代码 | 本次验证边界 |
|---|---|---|
| P01 | 共享日期协议与可选执行控制，保持核心入口 | AST 对照与静态审查；核心回归未运行 |
| P02/P03 | SQLite、严格模型、数据快照、预览及幂等提交 | 静态检查；存储与 API 用例已写、未运行 |
| P04/P05 | 独立 worker、取消/超时、真实核心适配、发布与恢复 | 独立静态审查；进程与故障注入未运行 |
| P06/P07 | 历史只读导入、重放、结果/比较服务、FastAPI 契约 | 静态审查和注解类型生成；OpenAPI 动态导出未运行 |
| P08/P09 | React 全部路由、草稿/幂等恢复、任务、图表、比较 | TypeScript 无输出检查；浏览器与 Vitest 未运行 |
| P10 | 新本地启动器候选、依赖分组、维护文档 | 入口源代码审查；未构建交付包，保留可选旧 UI |
| P11 | 静态检查工具、回归测试源和独立审查记录 | 不勾选动态 AC，不切 main、不推送 |

原计划中的 Playwright 端到端与完整安装/迁移演练尚未实施执行；此轮未提供已验证的浏览器自动化交付。详细本地审查记录放在被忽略的 `docs1/development/react-fastapi/`，正式使用文档不依赖该目录。
