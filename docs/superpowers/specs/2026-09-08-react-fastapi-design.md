...
- 状态：用户已后续授权实施；本次只编写代码和静态检查，动态验收未执行。
- 日期：2026-09-08；基线：`a66ea1bc6f1cf7a8c5512dc0454634b33304a0ab`。
- 实施分支：`refactor/react-fastapi`，一个独立工作树，不推送远端；原文档提案保存在 `docs/react-fastapi-refactor`。
...
首版定位为本地单用户研究工具，默认仅执行一个训练或重放任务；等待任务按队列排列。本次按此范围实施，执行和验收调整见 [ADR-0006](../../adr/0006-static-only-implementation.md)。
...
本 Spec 原为实施前提案。用户后续已明确要求开发，同时限制为静态检查和本地分支交付。该授权不包括推送代码、合并 main 或执行动态验收；FR-10 的最终旧入口清理继续等待真实验收证据。
